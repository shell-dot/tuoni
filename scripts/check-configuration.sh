...
config_example_path="$PROJECT_ROOT/config/example/example.tuoni.yml"
...
if [ $(yq e '. | has("client")' $config_path) == false ]; then
  echo "INFO | 'client' attribute is missing from config, adding it..."
  # Extract the 'client' node from the example file
  yq e '.client = load("'$config_example_path'").client' $config_path --inplace
fi
