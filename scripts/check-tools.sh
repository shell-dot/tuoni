...
if [ ! -f "$PROJECT_ROOT/scripts/tools/yq" ]; then
    ...
    ${TUONI_SUDO_COMMAND} docker run --rm -v "$PROJECT_ROOT/scripts/tools:/tools" "${TUONI_UTILITY_IMAGE}" cp /usr/bin/yq /tools/yq
    
    # Make the yq tool executable
    ${TUONI_SUDO_COMMAND} chmod +x "$PROJECT_ROOT/scripts/tools/yq"
